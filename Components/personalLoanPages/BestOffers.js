...
import { StyleSheet, Image, Text, View, StatusBar, TextInput, TouchableOpacity, Alert } from 'react-native';
...
export default function BestOffers({ route }) {
  ...
  const handleFullnameChange = (fullnameText) => { setFullname(fullnameText) };
  const handleEmailAddressChange = (emailAddressText) => { setEmailAddress(emailAddressText) };
  ...
  const pickImage = async () => {
    ...
    if (!result.cancelled) {
      setImage(result.uri);
      await AsyncStorage.setItem('photoDetails', result.uri)
    }
  };
      ...
      <View style={{ paddingLeft: 20, paddingRight: 20 }}>
        ...
        <View style={{ width: '100%', marginTop: -20 }}>
          ...
          <View style={{ marginTop: 20 }}>
            ...
            <TouchableOpacity style={styles.uploadButtonOutside} onPress={() => pickImage()}>
